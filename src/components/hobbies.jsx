...
export default function hobbies() {
  ...
  const addItem = () => {
    ...
    if (newItem.trim() !== "") {
      ...
      setNewItem("");
      localStorage.setItem("hobbies", JSON.stringify([...hobbies, {'name' : newItem}]));
    }
  };

  useEffect(() => {
    const stored2 = localStorage.getItem("hobbies");
    console.log(stored2);
    if (stored2) {
        ...
        <div className="grid grid-flow-cols mt-4 grid-cols-1 gap-y-4 md:mr-20">
          {hobbies.map((hobby) => {
            return (
              <>
                <div
                  id={hobby.name}
                  className="flex px-3 text-white flex-row justify-between items-center bg-newblue rounded-sm py-2"
                >
          ...
          {hobbies.length < 3 && val == 0 && (
            <button
              ...
              className="flex px-3 text-white flex-row justify-center items-center bg-gradient-to-r from-newblue to-grad-b2 rounded-sm py-2 space-x-2"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
